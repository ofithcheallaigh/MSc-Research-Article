...
We can see from this analysis that once again, the Decision Tree Classifier and the K-Nearest Neighbour algorithms produce 100 per cent accuracy. \textcolor{red}{The fact that these models are consistently producing perfect results will require further investigation}. The LDA algorithm producing some of the lowest accuracy figures. 

This analysis shows that it is possible to detect the presence of an object as well as localise that object to a grid location. The next step will be to build neural networks that can be deployed onto a constrained device, allowing the system to be tested live.
